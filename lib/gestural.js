...
Gesture.prototype.appendData = function (d) { // TODO: move integration to post-processing
  if (!this.active || d.constructor.name !== "Array") return false;
  var _ = this._internals;

  _.timePrev = _.time || _.startTime;
  _.time = new Date().getTime();

  _.accel.set.apply(_.accel, d);
  _.deltaV.copy(_.accel).multiplyScalar((_.time - _.timePrev) / 1000);
  if (_.startFlag) _.velocity.sub(_.deltaV);
  _.velocity.add(_.deltaV);
  _.deltaD.copy(_.velocity).multiplyScalar((_.time - _.timePrev) / 1000);
  _.displacement.add(_.deltaD);

  this.acceleration.push([_.accel.x, _.accel.y, _.accel.z]);
  this.velocity.push([_.velocity.x, _.velocity.y, _.velocity.z]);
  this.displacement.push([_.displacement.x, _.displacement.y, _.displacement.z]);
  this.time.push(_.time - _.startTime);
  _.startFlag = false;

  this.geometry.vertices.push(_.displacement.clone());
  return d
};
...
addEvents({
  "": {
    devicemotion: function (e) {
      if (gesture.active) $("#acceldata").textContent = gesture.appendData([e.acceleration.x, e.acceleration.y, e.acceleration.z])
    },
    deviceorientation: function (e) {
      if (gesture.active) $("#orientdata").textContent = [e.beta, e.gamma, e.alpha]
    },
    click: function () {
      ...
      if (gesture.active) {
        ...
        var adjD,
            accel = new THREE.Vector3().copy(gesture._internals.displacement)
              .multiplyScalar( 2 / Math.pow((gesture._internals.time - gesture._internals.startTime) / 1000, 2) );

        for (var i = 1; i < gesture.time.length - 1; i++) {
          adjD = accel.clone().multiplyScalar( Math.pow(gesture.time[i] / 1000, 2) / 2 );
          gesture.geometry.vertices[i+1].sub(adjD);
        }
        gesture.geometry.vertices[0].multiplyScalar(0);
        gesture.geometry.vertices[i+1].multiplyScalar(0);
